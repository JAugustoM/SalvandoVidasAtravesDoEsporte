...
void main() {
  ...
  group('AlunoExpandableCard Widget Test', () {
    testWidgets('Não deve renderizar nada se o aluno estiver inativo', (WidgetTester tester) async {
    ...
    testWidgets('Deve mostrar informações básicas quando não expandido', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(alunoAtivo));

      expect(find.text(alunoAtivo.nome), findsOneWidget);
      expect(find.text('Turma: ${alunoAtivo.idTurma ?? "N/A"}'), findsOneWidget);
    ...
    testWidgets('Deve expandir e mostrar detalhes ao ser clicado', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(alunoAtivo));
    ...
    testWidgets('Deve navegar para a página de edição ao clicar no botão "Editar"', (WidgetTester tester) async {
