...
void main() {
  group('HistoricoEmprestimosStore Tests', () {
    ...
    test(
      'build e updateFiltroTipo cobrindo empréstimos e devoluções',
      () async {
        ...
        when(
          mockKimonoService.listarEmprestimos(),
        ...
        final state = await container.read(
          historicoEmprestimosStoreProvider.future,
        );
        expect(state.itens.length, 2);
        expect(state.filtroTipo, 'todos');

        final notifier = container.read(
          historicoEmprestimosStoreProvider.notifier,
        );

        // Filtro emprestimo (dataDevolucao == null)
        notifier.updateFiltroTipo('emprestimo');
        final sEmprestimo = container
            .read(historicoEmprestimosStoreProvider)
            .value!;
