# Reuniões com Clientes

=== "Reunião 2 - 24/04"

    ## R2 - Reunião 24/04 - Brainstorming
    
    <iframe src="https://drive.google.com/file/d/1WLh7UAJ0mWn-l7rfomXp-mUDUPTXNiop/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - Giovani - 241032500
        - José Augusto - 231026429
        - Júlia - 241036142
        - Lucas - 241011386
        - Pedro Lucas - 241025710
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
        - João (Voluntário do Projeto Social)
        - Aline (Voluntária do Projeto Social)
    - **Monitor**:
        - Maykon
    
    ### Resumo
    
    Esta reunião foi dedicada à apresentação de todo o grupo aos stakeholders do projeto e à discussão sobre o cenário atual do projeto social. Isso permitiu a elicitação e a descoberta de requisitos por meio da técnica de *Brainstorming*.

=== "Reunião 3 - 08/05"

    ## R3 - Reunião 08/05 - Workshop JAD adaptado
    
    <iframe src="https://drive.google.com/file/d/1ildVTaRYth3Ttjt_PxKqnVIXsGviu6dn/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - Giovani - 241032500
        - José Augusto - 231026429
        - Júlia - 241036142
        - Lucas - 241011386
        - Pedro Lucas - 241025710
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
        - Aline (Voluntária do Projeto Social)
        - João (Voluntário do Projeto Social)
        - Leonardo (Voluntário do Projeto Social)
    
    ### Resumo
    
    Esta reunião foi dedicada à realização de uma sessão adaptada de Workshop JAD, na qual foram exercitadas as atividades de Elicitação e Descoberta, além de Análise e Consenso.

=== "Reunião 4 - 14/05"

    ## R4 - Reunião 14/05 - Validação do Backlog
    
    <iframe src="https://drive.google.com/file/d/1RTyCexbizr42eh2_47-xg4FG1LoyvHuI/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - José Augusto - 231026429
        - Júlia - 241036142
        - Lucas - 241011386
        - Pedro Lucas - 241025710
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
        - Aline (Voluntária do Projeto Social)
        - Leonardo (Voluntário do Projeto Social)
    
    ### Resumo
    
    Esta reunião foi dedicada à validação do backlog proposto pela equipe de desenvolvimento, com base nas informações coletadas durante o Workshop JAD e nas demais reuniões. A validação incluiu os requisitos e o valor de negócio associado a cada um deles.

=== "Reunião 5 - 27/05"

    ## R5 - Reunião 27/05 - Validação de atualizações do DoR e DoD
    
    <iframe src="https://drive.google.com/file/d/1tzqKHv4k9UYIYuEdwx6zHskU2wFJqzN3/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - Júlia - 241036142
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
    
    ### Resumo
    
    Esta reunião foi dedicada à validação das atualizações do *Definition of Ready* (DoR) e do *Definition of Done* (DoD), incluindo uma explicação prática ao cliente sobre como esses critérios funcionarão durante o andamento do projeto. Além disso, foi validado o escopo da implementação inicial, com a demonstração e a revisão dos protótipos das telas de login e de cadastro de voluntários.

=== "Reunião 6 - 29/05"

    ## R6 - Reunião 29/05 - Validação da primeira versão do aplicativo e de protótipos
    
    <iframe src="https://drive.google.com/file/d/12wdv-cJvUM6hfnxGGuXK3o0w4dbRnLRP/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - Giovani - 241032500
        - José Augusto - 231026429
        - Júlia - 241036142
        - Lucas - 241011386
        - Pedro Lucas - 241025710
        - Pedro Ramos - 222031680
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
        - Aline (Voluntária do Projeto Social)
        - João (Voluntário do Projeto Social)
    
    ### Resumo
    
    Nesta reunião, foram realizadas as validações da primeira versão do aplicativo — que contava apenas com a tela de login e o cadastro de voluntários — e dos protótipos de baixa fidelidade da página inicial (dashboard) e do cadastro de alunos. 
    
    Em relação à primeira versão do aplicativo, os voluntários e o representante aprovaram o aspecto visual, cientes de que ainda se tratava de uma versão inicial. No entanto, solicitaram a adição de um campo para CPF no cadastro de voluntários.
    
    Os stakeholders também tiveram uma impressão positiva dos protótipos apresentados, mas reforçaram a necessidade de acrescentar uma aba para coletar informações médicas básicas, que não estava presente na versão demonstrada.

=== "Reunião 7 - 03/06"

    ## R7 - Reunião 03/06 - Segunda sessão JAD: priorização das User Stories para composição do MVP 
    
    <iframe src="https://drive.google.com/file/d/1ANG92X-n4NnMNAb-J_wUUmgj6HJkW9p_/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - Giovani - 241032500
        - José Augusto - 231026429
        - Júlia - 241036142
        - Lucas - 241011386
        - Pedro Lucas - 241025710
        - Pedro Ramos - 222031680
    - **Stakeholders**:
        - José Lucas (Representante do Projeto Social)
        - Aline (Voluntária do Projeto Social)
        - Leonardo (Voluntário do Projeto Social)
    
    ### Resumo
    
    Nesta reunião, foi realizada a segunda sessão JAD, com o foco de aplicar novamente a técnica MoSCoW com os stakeholders, desta vez utilizando as histórias de usuário. No geral, a priorização dos requisitos manteve-se semelhante à anterior, com apenas algumas alterações. Em especial, a US20 (relacionada ao registro de orientações pedagógicas) teve sua prioridade elevada para *Should*.

=== "Reunião 8 - 11/06"

    ## R8 - Reunião 11/06 - Validação final da entrega da Unidade 3 com o cliente
    
    <iframe src="https://drive.google.com/file/d/1tVHLPk6nORJZQIJz4lndP1cXXA0Hee4x/preview" width="640" height="480"></iframe>
    
    ### Participantes
    
    - **Equipe**:
        - José Augusto - 231026429
        - Júlia - 241036142
        - Pedro Lucas - 241025710
    - **Stakeholders**:
        - Aline (Voluntária do Projeto Social)
        - Leonardo (Voluntário do Projeto Social)
    
    ### Resumo
    
    Nesta reunião, foram validados os protótipos de média fidelidade do dashboard principal, do cadastro de alunos, da página de turmas e da página de registro de presença. Também foi validada a segunda versão do aplicativo.
    
    Os protótipos foram aceitos com apenas pequenas alterações. No dashboard principal, especificamente, os voluntários afirmaram que não seria necessária a inclusão do número de turmas, nem a indicação de quais alunos específicos possuem um kimono doado.
