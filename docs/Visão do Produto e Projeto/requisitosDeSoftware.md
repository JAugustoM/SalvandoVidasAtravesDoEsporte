...
| RNF04 | Proteção de dados sensíveis e conformidade com a LGPD | O sistema deve implementar mecanismos de proteção de dados pessoais dos alunos e responsáveis, garantindo conformidade total com a Lei Geral de Proteção de Dados Pessoais. | Segurança |
...
| RNF10 | O sistema deve invalidar a sessão automaticamente e redirecionar para a tela de login após [X período] de inatividade | O sistema deve disponibilizar uma opção de "modo escuro" alternável pelo usuário, aplicando uma paleta de cores de fundo escuro e textos de alto contraste em todas as telas, preservando a legibilidade e a consistência visual. | Usabilidade |
